...
#DAVILA BAILON JOSE GILBERTO EXPEDIENTE:220209301
#
#
#
...
@app.route("/invitado")
def paginainvitado():
    render_template("invitado.html")


@app.route("/Dar_renta/", methods=['POST','GET'])
...
@app.route("/busc_local/", methods=['GET'])
def dar_lista():
    if request.method == 'GET':
        dic_citas = lee_diccionario_csv_id('receta_completa.csv')
        return render_template("lista_locales.html",dicc_citas=dic_citas)
...
@app.route("/rentas_ant/", methods=['GET','POST'])
def citas():
     if request.method == 'GET':
...
@app.route("/Vendedores_Ant/", methods=['GET'])
def lista_citas():
    if request.method == 'GET':
        dic_citas = lee_diccionario_csv_id(archivo_citas)
        return render_template("lista_vendedores.html",dicc_citas=dic_citas)
...
def lista_receta_usuario():
    if request.method == 'GET':
        dic_citas = lee_diccionario_csv_id('receta_completa.csv')
        return render_template("lista_casa.html",dicc_citas=dic_citas)
